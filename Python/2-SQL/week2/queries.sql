...
SELECT c.category_id,c.category_name,c.description,c.picture from categories c
...
SELECT e.first_name, e.last_name, e.reports_to FROM employees e 
...
SELECT o.ship_address, COUNT(o.order_id) AS order_count FROM orders o
...
SELECT o.customer_id, SUM(o.freight) FROM orders o
GROUP BY o.customer_id
...
WITH shippers_per_customer AS (
    SELECT COUNT(DISTINCT o.ship_via) as shipper_count from orders o
    GROUP BY o.customer_id
) 
...
SELECT s.phone FROM suppliers s
UNION
